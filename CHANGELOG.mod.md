...
- Invalid instrument regions are skipped with a warning instead of rejecting the whole SoundFont. `SoundFontError::SanityCheckFailed` is no longer returned. Regions that never loop keep playing even when their loop points are unusable, since a non-looping region never reads them.
