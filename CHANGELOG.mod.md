...
- SysEx processing via `Synthesizer::process_sysex`:
  ...
  - GS Patch Part receive switches (Rx. Pitch Bend, Channel Pressure, Program Change, Control Change, Poly Pressure, Note, RPN, NRPN, Modulation, Volume, Panpot, Expression, Hold 1, Portamento, Sostenuto, Soft, Bank Select and Bank Select LSB). A message a part does not receive is discarded before it reaches any channel state. All switches start on and a reset message turns them all back on, so a file that sends none behaves as before; GS powers up with Rx. NRPN off, which is not followed here, because this is not a GS device and XG files use NRPN without sending a GS Reset.
  - GS Assign Mode. In Single mode a note struck again while it still sounds stops the one already playing instead of layering another on top. Limited-Multi and Full-Multi both let repeats layer, because the documentation names the modes without saying where Limited-Multi draws its limit.
  - GS Pitch Offset Fine, which detunes a part by a fixed number of hertz rather than by a ratio, so it shifts a low note by a far wider interval than a high one.
...
- `Synthesizer::reset_effect_parameters` restores the reverb and chorus parameters to the library defaults, for a host that has no defaults of its own to restore when it starts a new song.
...
- Effect parameters set by GS reverb/chorus macros persist across `Synthesizer::reset`, like parameters set through the API. The GM/GS/XG reset messages do restore the defaults, so a reverb or chorus character selected by one song does not leak into the next one; `Synthesizer::reset_effect_parameters` does the same on demand.
